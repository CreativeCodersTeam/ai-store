...
description: >
  ...
  locally diverged; for a diverged file it derives what changed upstream, checks the merged result
  for contradictions, resolves every conflict with the user, and writes only after approval. Never
  commits. Must NOT activate on its own for "check my CLAUDE.md", "improve CLAUDE.md", "audit
  project memory", or any other CLAUDE.md quality review — those judge the file's content, this
  only syncs it with its template.
---
...
guesswork. Offer the honest alternative — treat the local file as unrelated and merely list what
the current template contains that the local file lacks — and let the user choose.
...
2. Write the new content to the target path. **Copy it from a file, never retype it.** On the
   `known-older-version` and `no-local-file` paths that file is `files.current`; on the `diverged`
   path it is the merged draft you produced in Step 6 and the user approved in Step 8, written out
   once and then copied. Reproducing a governing file from memory, or from the block quoted in your
...
nothing was committed. If Step 2 reported `stale: true`, repeat that warning here — it is the
...
skill is that a human ruled on the conflicts.
